...
void sha256_process_arm(uint32_t state[8], const uint8_t data[], uint32_t length)
{
    ...
    if(num_bits_padding < 0){
        num_bits_padding += 512;
    }

    uint16_t bytes_padding = ( num_bits_padding + 1) / 8;
    uint8_t padding[bytes_padding] = {0};
    padding[0] = 1<<7;

    uint8_t padding_length[128] = {0};

    memcpy(&padding_length[0], &data[length - (length % 64)], length%64);

    memcpy(&padding_length[length%64], &padding[0], 1);

    for(int i = 8; i > 0; i--){
        uint8_t current_byte = in_bits >> ((i-1) << 3);
        memcpy(&padding_length[length % 64 + bytes_padding+(8-i)], &current_byte, 1);
    }
    ...
    while (length >= 64)
    {
        ...
        SHA256_comp(MSG0, MSG1, MSG2, MSG3, &STATE0, &STATE1, &ABEF_SAVE, &CDGH_SAVE);
    ...
    /* Load message */
    MSG0 = vld1q_u32((const uint32_t *)(padding_length +  0));
    MSG1 = vld1q_u32((const uint32_t *)(padding_length + 16));
    MSG2 = vld1q_u32((const uint32_t *)(padding_length + 32));
    MSG3 = vld1q_u32((const uint32_t *)(padding_length + 48));
    ...
    MSG0 = vld1q_u32((const uint32_t *)(padding_length + 64));
    MSG1 = vld1q_u32((const uint32_t *)(padding_length + 80));
    MSG2 = vld1q_u32((const uint32_t *)(padding_length + 96));
    MSG3 = vld1q_u32((const uint32_t *)(padding_length + 112));
...
void SHA256_NEON(chunk_t *chunk)//, wc_Sha3* sha3_384)
{
    ...
    sha256_process_arm(state, chunk->start, chunk->length);
